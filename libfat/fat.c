...
struct fat_bpb * parse_bpb(){
	struct fat_bpb *ret = kmalloc(sizeof(struct fat_bpb *) * sizeof(*ret));
	char *buf = kmalloc(1024);
	if(ata_read_master(buf,0,0) < 0){
		kprintf("Couldn't read from drive!\n");
		panic();
	}
...
struct fat16_ebr *parse_fat16_ebr(){
	...
	struct fat16_ebr *ret = kmalloc(sizeof(struct fat16_ebr *) * sizeof(*ret));
	ret->drive_number = buf[36];
	ret->flags_nt = buf[37];
	ret->sig = buf[38];
	if(ret->sig != 0x28 && ret->sig != 0x29){
		kprintf("Invalid ebr %d!\n",buf[38]);
		panic();
	}
...
struct fat32_ebr *parse_fat32_ebr(){
	...
	struct fat32_ebr *ret = kmalloc(sizeof(struct fat32_ebr *) * sizeof(*ret));
...
struct info *fat_parse_info(){
	...
	struct info *ret = kmalloc(sizeof(struct info *) * sizeof(*ret));
...
int read_root(int type){
	struct info *inf = fat_parse_info();
	struct fat_dirent *dent = kmalloc(sizeof(struct fat_dirent *) * sizeof(*dent));
